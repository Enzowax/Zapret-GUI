...
![Простой режим — всё нужное на одном экране](docs/screenshot-simple.png)
...
- **Простой** — четыре понятных пункта меню, ничего лишнего:
  - **Главная** — большие кнопки **Запустить / Остановить**, «здоровье»
    Discord / YouTube / Google, Telegram в один клик и **«Подобрать и включить»**
    (сама найдёт рабочую стратегию и запустит);
  - **Свои сайты** — домены для обхода + переключатель **«Не трогать Steam / Dota 2»**;
  - **Настройки** — тема, **выбор акцентного цвета**, автозапуск с Windows, обновления;
  - **Журнал**.

  Поставил, нажал — работает.
- **Полный** — всё для тонкой настройки: пресеты, служба Windows, свои сайты,
  DoH, диагностика, журнал, автообновления. Ничего не урезано — просто скрыто
  от новичков.

## ✨ Возможности

- **Мастер первого запуска** — выбор режима и автоматическая первичная настройка
  без единой ручной операции.
- **Дашборд на главной** — статус обхода (с именем активной стратегии), живое
  «здоровье» (Discord / YouTube / Google + Telegram-прокси) и Старт/Стоп плиткой.
- **Умный авто-поиск** стратегии: пресеты пробуются по приоритету (последний
  ...
  остановкой. Проверка — **с верификацией TLS-сертификата**, поэтому заглушка
  цензора не может ложно засчитаться как рабочая стратегия.
...
Исправлено в 2.38.0. Раньше проверка засчитывала любое завершённое
TLS-рукопожатие, поэтому соединение, которое цензор заворачивал на свою заглушку,
ложно считалось рабочим — да ещё и с минимальной задержкой попадало в «лучшие»
(типично для `general (ALT)`). Теперь проверка **верифицирует сертификат и имя
хоста**: заглушка не проходит, в списке остаются только по-настоящему рабочие
стратегии. Если обновились с более старой версии — прогоните авто-поиск ещё раз.
</details>
...
Обычно причина — задача автозапуска осталась со старым путём после переноса
папки или обновления. С 2.39.1 приложение **само чинит это при запуске**:
проверяет, что задача указывает на текущий `.exe`, и молча перерегистрирует её.
Достаточно один раз запустить приложение вручную. Если не помогло — выключите и
снова включите тумблер **«Запускаться вместе с Windows»** в настройках.
</details>

<details>
<summary><b>Не копируется/вставляется в поля при русской раскладке</b></summary>

Исправлено в 2.39.2. Ctrl+C/V/X/A теперь ловятся по позиции клавиши, а не по
символу, поэтому работают при любой раскладке — в «Своих сайтах», фильтре
журнала и полях Telegram-прокси.
</details>
